...
int ledSavedG = 0;
int ledSavedB = 80;
int ledSavedBright = 80;
bool ledPoweredOff = true;
...
void drawClockHeaderWidget();              // V4 app: hora arriba izquierda, bateria arriba derecha
...
void bleRemoteSetWifiWebFtp(bool on);
...
void bleRemoteSetWifiWebFtp(bool on) {
  // V4 App: un solo comando BLE enciende o apaga WiFi + Web/OTA + FTP.
  if (on) {
    if (!bleRemoteEnsureWifiConnected()) {
      bleRemoteSendStatus();
      return;
    }
    webServerEnabled = true;
    webServerStatusMsg = "Web activa por app V4";
    otaStartIfPossible();
    if (!ftpEnabled) ftpStartStop();
    bleRemoteNotify(String("WIFI_FULL|ON|") + WiFi.localIP().toString());
  } else {
    bleRemoteSetFtp(false);
    bleRemoteSetWeb(false);
    wifiManualOff();
    bleRemoteNotify("WIFI_FULL|OFF");
  }
  bleRemoteSendStatus();
}
...
void bleRemoteProcessCommand(String cmd) {
  ...
  if (cmd == "GET_STATUS" || cmd == "GET_STATE") { bleRemoteSendStatus(); return; }

  if (cmd == "WIFI_FULL_ON" || cmd == "START_SERVERS" || cmd == "ALL_ON") { bleRemoteSetWifiWebFtp(true); return; }
  if (cmd == "WIFI_FULL_OFF" || cmd == "ALL_OFF") { bleRemoteSetWifiWebFtp(false); return; }
...
void setup() {
  ...
  delay(bootDelay);
  Serial.println();
  Serial.println("Control_ESP32_S3_V4_App_BLE_WIFI_OTA arrancando...");
  ...
  ws2812Init();
  // V4: el RGB debe arrancar apagado. Solo se enciende por pagina LED
  // o por actividad WiFi/microSD. Evita la luz azul fija al arrancar.
  ledPoweredOff = true;
...
void showSplash() {
  ...
  tft.drawCentreString("Control ESP32-S3 V4 BLE", SCREEN_W / 2, 92, 2);
...
void drawHeader(const char *title) {
  ...
  // V4 App: hora en la esquina superior izquierda y bateria en la derecha.
  drawClockHeaderWidget();
